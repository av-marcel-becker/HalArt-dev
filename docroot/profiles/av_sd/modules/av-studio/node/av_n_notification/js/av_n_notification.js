class AVNotification extends HTMLElement {
    ...
    connectedCallback() {
        ...
        const closeOnBackgroundClick = this.getAttribute('close-on-background-click') !== 'false';
        if (closeOnBackgroundClick) {
            this.shadowRoot.querySelector('.av-notification-background').addEventListener('click', (event) => {
                if (event.target === this.shadowRoot.querySelector('.av-notification-background')) {
                    this.close();
                }
            });
        }
        ...
        const displayMode = this.getAttribute('display-mode') || 'auto';
        const now = Date.now();
        ...
        window.addEventListener('hashchange', () => this.handleAnchor());
    }

    handleAnchor() {
        const anchor = location.hash.substring(1);
        const notificationAnchor = this.getAttribute('anchor');
